...
#   - mask[i,j] = 1 (high similarity) → FP8 (similar frames, less precision needed)
#   - mask[i,j] = 0 (low similarity)  → BF16 (important frame pairs, need precision)
...
class FastGlobalAttention(Attention):
    ...
    def forward(
        ...
        if frame_mask is not None and num_frames is not None:
            ...
            num_patches_per_frame = N // num_frames

            # time for standard scaled dot-product attention with torch cuda synchronization
            ...
            end_event.record()
            torch.cuda.synchronize()
            standard_time = start_event.elapsed_time(end_event)
            print(f"Standard scaled dot-product attention time: {standard_time} ms for {q.shape}, q.dtype: {q.dtype}, k.dtype: {k.dtype}, v.dtype: {v.dtype}")

            torch.cuda.synchronize()
            start_event = torch.cuda.Event(enable_timing=True)
            end_event = torch.cuda.Event(enable_timing=True)
            start_event.record()
            # out = mixed_precision_attention(
            #     q, k, v, 
            #     frame_mask=frame_mask,
            #     num_patches_per_frame=num_patches_per_frame,
            # )

            out = attention(q, k, v, False, self.scale, False)
            end_event.record()
            torch.cuda.synchronize()
...
def compute_frame_saliency(
    ...
    mask = (mask >= 0.9).to(torch.uint8) # hardcoding threshold for better torch.compile
    return mask
...
def _process_global_attention(self, tokens, B, S, P, C, global_idx, pos=None, frame_mask=None):
        ...
        for _ in range(self.aa_block_size):
            ...
            if isinstance(block.attn, FastGlobalAttention):
                attn_out = block.attn(block.norm1(x), pos=pos, frame_mask=frame_mask, num_frames=S)
            else:
